...
import { buySeatLevelSix } from "../../contractInteractions/Ticket";
...
const TicketCard = ({
  ...
  const ticketMinted = async (): Promise<void> => {
    ...
    try {
      const count = await TicketContract.homeSeatLevelCount(6);
      const countTwo = await TicketContract.awaySeatLevelCount(6);
    ...
    <section className="h-[480px] max-w-[290px] bg-[#F5F5F5] px-3 md:px-6 pb-6 pt-8 my-16 mx-10 rounded-xl">
      ...
      <div className="flex justify-around mb-5">
        ...
        <h2 className="text-black flex items-center ">
          ...
          />
          <p className="ml-1">0.5 MATIC</p>
        </h2>
      </div>
      <div className="flex flex-col justify-center items-center">
        <p className="text-black mb-3">Sixth Row Seat</p>
        ...
        <button
          onClick={() =>
            buySeatLevelSix(homeorAway === "Home" ? true : false, signer)
          }
