...
import { buySeatLevelTwo } from "../../contractInteractions/Ticket";
...
const TicketCard = ({
  ...
  const ticketMinted = async (): Promise<void> => {
    ...
    try {
      const count = await TicketContract.homeSeatLevelCount(2);
      const countTwo = await TicketContract.awaySeatLevelCount(2);
    ...
    <section className="h-[480px] max-w-[290px] bg-[#F5F5F5] px-3 md:px-6 pb-6 pt-8 my-16 mx-10 rounded-xl">
      ...
      <div className="flex justify-around mb-5">
        ...
        <h2 className="text-black flex items-center ">
          ...
          />
          <p className="ml-1">4 SFUEL</p>
        </h2>
      </div>
      <div className="flex flex-col justify-center items-center">
        <p className="text-black mb-3">Second Row Seat</p>
        ...
        <button
          onClick={() =>
            buySeatLevelTwo(homeorAway === "Home" ? true : false, signer)
          }
