...
describe("Ticket", async function () {
  // Enum similar to the one in contract
  enum side {
    HOME,
    AWAY,
  }

  it("Should be able to resell ticket to other address", async function () {
    ...
    await ticket
      .connect(addr1)
      .buyTicket(side.HOME, 1, { value: ethers.utils.parseEther(`5`) });
    expect(await ticket.balanceOf(addr1.address)).to.equal(1);
    expect(await ticket.ownerOf(1)).to.equal(addr1.address);
    await ticket
      .connect(addr2)
      .buyTicket(side.AWAY, 1, { value: ethers.utils.parseEther(`5`) });
  ...
  it("Should be able to buy ticket from first seat level", async function () {
    ...
    const resell = await Resell.connect(owner).deploy(
      100000000000000,
      ticket.address
    );
    ...
    await ticket
      .connect(addr1)
      .buyTicket(side.AWAY, 1, { value: ethers.utils.parseEther(`5`) });
    const awayTicketsOwned = await ticket
      .connect(addr1)
      .getAwayTicketsCountByOwner(1);
    await ticket.connect(addr1).approve(addr2.address, 101);
    await resell
      .connect(addr2)
      .receiveTicketLevelOne(false, 101, addr1.address, {
        value: ethers.utils.parseEther(`5`),
      });
    const awayTicketsOwnedTwo = await ticket
      .connect(addr1)
      .getAwayTicketsCountByOwner(1);
    console.log("First", awayTicketsOwned);
    console.log("Second", awayTicketsOwnedTwo);
    console.log(await ticket.connect(addr2).getAwayTicketsCountByOwner(1));
    console.log(await ticket.connect(addr2).getHomeTicketsCountByOwner(1));
  });
  ...
  it("Should be able to buy ticket from first seat level", async function () {
    ...
    const refund = await Refund.connect(owner).deploy(
      100000000000000,
      ticket.address
    );
  ...
  it("Should be able to buy ticket from first seat level", async function () {
    ...
    await refund.deployed();
    await ticket.connect(owner).setRefundContract(refund.address);
    const RefundTicket = await ethers.getContractFactory(
      "ReceiveRefundedTicketFromContract"
    );
    const refundTicket = await RefundTicket.connect(owner).deploy(
      100000000000000,
      ticket.address
    );
    ...
    await ticket
      .connect(owner)
      .setReceivedRefundTicketContract(refundTicket.address);
    await ticket
      .connect(addr1)
      .buyTicket(side.AWAY, 1, { value: ethers.utils.parseEther(`5`) });
    await refund.connect(addr1).refundBackToContractLevelOne(false, 101);
    await refundTicket.connect(addr2).refundTicketLevelOne(false, 101, {
      value: ethers.utils.parseEther(`5`),
    });
  });
});
